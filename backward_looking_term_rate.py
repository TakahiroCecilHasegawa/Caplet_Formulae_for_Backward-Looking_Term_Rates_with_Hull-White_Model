...
zero                      = 0.01
historical_overnight_rate = 0.01

# Evaluation parameters
today                  = '2021/04/01'
application_start_date = '2021/05/01'
...
aylst_normal    = np.random.normal(mu_num, math.sqrt(val_num), 1000000000)
aylst_lognormal = np.exp(aylst_normal)
...
for strike_weight in strike_weight_list:
    ...
    caplet = caplet_pricer(today, underlying, strike, application_start_date, application_end_date,True)
    print("Analytical solution for Comp.", str(caplet))
    ...
    payoff_comp_mean = payoff_comp.mean()
    print("MonteCarlo solution for Comp.", str(payoff_comp_mean))
    diff = (payoff_comp_mean/caplet - 1.0)
    print("Diff(MC/AS - 1) for Comp.    ", str(diff))
    print()
...
for strike_weight in strike_weight_list:
    strike = underlying * strike_weight
    print("[underlying, strike] = ", str([underlying, strike]))
    caplet = caplet_pricer(today, underlying, strike, application_start_date, application_end_date, False)
    print("Analytical solution for AA .", str(caplet))

    _tau = (date_to_serial(application_end_date) - date_to_serial(application_start_date))/ACT365FIX
    adj_strike = _tau*strike
    payoff_aa = aylst_normal - adj_strike
    payoff_aa[payoff_aa < 0.0] = 0.0
    payoff_aa_mean = payoff_aa.mean()
    print("MonteCarlo solution for AA .", str(payoff_aa_mean))
    diff = (payoff_aa_mean/caplet - 1.0)
    print("Diff(MC/AS - 1) for AA      ", str(diff))
    print()
